...
# 9 Проверяем что незаполненные обязательные поля не дают возможности добавить питомца - в работе
# В swagger присутствует проверка на ввод
def test_add_new_pet_with_no_data(name='', animal_type='', age='',
    ...
    # Сверяем полученный ответ с ожидаемым результатом
    assert status == 200
    assert 'id' not in result  # как проверить работу теста?


# 10 Проверяем что поле ввода имени питомца ограничено 32 символами (в тестовом swagger нет ограничения)
def test_add_new_pet_no_data(name=f'Test'*1024, animal_type='Собака', age='1',
    ...
    if len(result['name']) < 33:

        assert status == 200
        print('len', len(result['name']), result)
        assert len(['name']) > 0

    else:
        # если имя питомца превышает 32 символа, то выкидываем исключение с текстом имя не должно превышать 32 символа
        raise Exception("Name must not exceed 32 characters")
